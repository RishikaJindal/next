...
// console.log( StringCalculate( ("2*2")));
function parse(str){
    return Function (`'use strict';return(${str})`)()
...
function StringCalculate(string){
    ...
    for (let i=0;i<splittedString.length; i++){
    if(i !==0 && i!==splittedString.length-1){
        if(splittedString[i]===')'&& (splittedString[i+1]==Number(splittedString[i+1]|| splittedString[i-1]=='(')&& flag==0)){
            evaluateExpression +=splittedString[i] +"*";
            flag=true;
        }
       
           else if(splittedString[i]==='('&& (splittedString[i-1]==Number(splittedString[i-1]|| splittedString[i+1]==')')&& flag==0)){
                evaluateExpression +="*"+splittedString[i] ;
                flag=true;
            }
       else evaluateExpression +=splittedString[i];
